                                    ...
                                    @if(in_array($attendance->user->id, $pendingConnectionRequestUserIds))
                                        <div class="flex-shrink-0">
                                            <form method="POST" action="{{ route('connections.cancel', $attendance->user) }}" class="inline">
                                                @csrf
                                                <button type="submit"
                                                        class="py-2 px-3 w-full inline-flex justify-center items-center gap-x-2 text-sm font-medium text-nowrap rounded-xl border border-transparent bg-red-400 text-black hover:bg-red-500 focus:outline-hidden focus:bg-yellow-500 transition disabled:opacity-50 disabled:pointer-events-none cursor-pointer">
                                                    Cancel
                                                </button>
                                            </form>
                                        </div>
